...
export class HtmlToPdfmakeService {
  ...
  convertHtmlToPdfmake(htmlText: string, options?: HtmlToPdfMakeOptions): any {
    ...
    // Font sizes converted from px to pt: 1px = 0.803571pt (reduced by ~25% for smaller fonts)
    this.fontSizes = options?.fontSizes ?? [6, 8.4, 9.6, 10.8, 12, 14.4, 16.8];
    ...
    // Font sizes converted from px to pt using ratio: 1px = 0.803571pt (reduced by ~25% for smaller fonts)
    this.defaultStyles = {
      b: { bold: true },
      strong: { bold: true },
      ...
      em: { italics: true },
      i: { italics: true },
      h1: { fontSize: 22, bold: true, margin: [0, 0, 0, 8], lineHeight: 1.5 },
      h2: { fontSize: 18, bold: true, margin: [0, 0, 0, 6], lineHeight: 1.5 },
      h3: { fontSize: 14.5, bold: true, margin: [0, 0, 0, 6], lineHeight: 1.5 },
      h4: { fontSize: 13.8, bold: true, margin: [0, 0, 0, 5], lineHeight: 1.5 },
      h5: { fontSize: 12.4, bold: true, margin: [0, 0, 0, 5], lineHeight: 1.5 },
      h6: { fontSize: 11, bold: true, margin: [0, 0, 0, 5], lineHeight: 1.5 },
      a: { color: 'blue', decoration: 'underline' },
      strike: { decoration: 'lineThrough' },
      p: { fontSize: 9.6, margin: [0, 0, 0, 5], lineHeight: 1.5 },
  ...
  private convertHtml(htmlText: string): any {
    ...
    // Preserve multiple consecutive spaces by replacing them with non-breaking spaces
    // This prevents the browser from collapsing them when parsing
    // Match text content between tags (not inside tag attributes)
    htmlText = htmlText.replace(/>([^<]+)</g, (match: string, textContent: string) => {
      // Replace 2 or more consecutive spaces with: first space + non-breaking spaces for the rest
      const preserved = textContent.replace(/([ \t]{2,})/g, (spaces: string) => {
    ...
    switch (element.nodeType) {
      case 3: { // TEXT_NODE
        if (element.textContent) {
          ...
          // Also check computed style for white-space property
          if (!hasWhiteSpace && parents.length > 0) {
            const parentElement = parents[parents.length - 1];
            if (parentElement && parentElement.nodeType === 1) { // ELEMENT_NODE
              try {
                const computedStyle = this.wndw.getComputedStyle(parentElement);
                const whiteSpace = computedStyle.whiteSpace || parentElement.style.whiteSpace;
                if (whiteSpace === 'pre' || whiteSpace === 'pre-wrap' || whiteSpace === 'pre-line' || whiteSpace === 'break-spaces') {
                  hasWhiteSpace = true;
                }
              } catch (e) {
          ...
          if (!hasWhiteSpace) {
            ...
            text = text.replace(/\s*\n\s*/g, ' ');
            // Normalize multiple spaces to single space, but preserve non-breaking spaces
            // Non-breaking spaces are used to preserve multiple consecutive spaces
            text = text.replace(/[ \t]+/g, ' ');
            // Don't replace non-breaking spaces - they're used to preserve spacing in PDF
            // PDFMake will render non-breaking spaces correctly
          } else {
            // When preserving whitespace, keep non-breaking spaces as-is
            // They help preserve spacing in PDFMake
          }
  ...
  private handleSpecialTags(element: any, ret: any, parents: any[], nodeName: string, nodeNameLowerCase: string): any {
    switch (nodeName) {
      ...
      case 'SUB':
      case 'SUP':
        ret[nodeNameLowerCase] = { offset: '30%', fontSize: 6 };
        return ret;
      case 'HR':
  ...
  private parseOtherStyles(key: string, value: string, ignoreProperties: boolean, 
    ...
    if (value) {
      ...
      if (key === 'fontSize' && parsedValue === false) {
        // Font size keywords converted using ratio: 1px = 0.803571pt (reduced by ~25% for smaller fonts)
        const fontSizeKeywords: { [key: string]: number } = {
          'xx-small': 4.3,    // 7.2px → 4.3pt (reduced)
          'x-small': 5.4,     // 9px → 5.4pt (reduced)
          'small': 6.5,       // 10.7px → 6.5pt (reduced)
          'medium': 7.2,      // 12px → 7.2pt (reduced)
          'large': 8.7,       // 14.4px → 8.7pt (reduced)
          'x-large': 10.8,    // 18px → 10.8pt (reduced)
          'xx-large': 14.5,   // 24px → 14.5pt (reduced)
          'xxx-large': 21.7   // 36px → 21.7pt (reduced)
        };
