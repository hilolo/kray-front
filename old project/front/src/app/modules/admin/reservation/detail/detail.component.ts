import { Component, OnInit, ViewEncapsulation, ChangeDetectorRef, OnDestroy, inject, HostListener, ElementRef, ViewChild } from '@angular/core';
...
export class ReservationDetailComponent implements OnInit, OnDestroy {
    ...
    ngOnInit(): void {
        ...
        this._route.params.pipe(takeUntil(this._unsubscribeAll)).subscribe(params => {
            if (params['id'] && params['id'] !== 'add') {
                ...
                this.isViewMode = true; // Set view mode for existing reservation
                this.loadReservation(this.reservationId);
            }
        });

        // Listen to date changes to calculate nights and sync datetime fields
        this.reservationForm.get('arrivalDate').valueChanges
        ...
        this.reservationForm.get('departureDate').valueChanges
            .pipe(takeUntil(this._unsubscribeAll))
            .subscribe(() => {
                this.syncEndDate();
                this.calculateNights();
    ...
    initializeForms(): void {
        ...
        this.notificationsForm = this._formBuilder.group({
            ...
            notificationWhatsappEntry: [false],
            notificationWhatsappExit: [false]
        });
    }


    /**
     * Load properties (only vacation rental properties)
     */
    loadProperties(): void {
        this._propertyService.getProperties({
            ...
            ignore: true,
            searchQuery: '',
            category: 2 // PropertyCategory.LocationVacances - Location vacances
        }).subscribe({
            next: (result) => {
                ...
                if (this.isEditMode && this.reservationForm.get('propertyId')?.value) {
    ...
    loadTenants(): void {
        this._contactsService.getContactsByType('tenant', 1, 1000, true).subscribe({
            next: (contacts) => {
                ...
                if (this.isEditMode && this.reservationForm.get('contactId')?.value) {
                    this.populateSearchTerms();
                }
    ...
    selectProperty(property: Property): void {
        this.reservationForm.patchValue({ propertyId: property.id });
        this.selectedPropertyForDisplay = property;
        this.propertySearchTerm = property.name || '';
        this.showPropertyDropdown = false;
        this._changeDetectorRef.markForCheck();
    }
    ...
    selectTenant(tenant: Contact): void {
        this.reservationForm.patchValue({ contactId: tenant.id });
     ...
     */
    clearProperty(): void {
        this.reservationForm.patchValue({ propertyId: '' });
     ...
     */
    clearTenant(): void {
        this.reservationForm.patchValue({ contactId: '' });
    ...
    getSelectedTenant(): Contact | null {
        const tenantId = this.reservationForm.get('contactId')?.value;
        ...
        setTimeout(() => {
            ...
            dialogRef.afterClosed().subscribe(result => {
                if (result && result.id) {
                    ...
                    // Check if property matches the category filter (category 2 = LocationVacances)
                    const propertyCategory = property.category;
                    const categoryMatch = propertyCategory === 2 || 
                                        propertyCategory === '2' || 
                                        propertyCategory === 'LocationVacances' ||
                                        Number(propertyCategory) === 2;
    ...
    loadReservation(id: string): void {
        ...
        this._reservationService.getReservationById(id).subscribe({
            next: (reservation) => {
                this.reservation = reservation;
                this.attachments = reservation.attachments || [];
                
                // Extract dates from startDate and endDate (ignore time)
                ...
                this.additionalInfoForm.patchValue({
                    ...
                    approvalNotes: reservation.approvalNotes || '',
                    privateNote: reservation.privateNote || '',
                    specialTerms: (reservation as any).specialTerms || ''
                });

                // Patch notifications form
                this.notificationsForm.patchValue({
                    enableNotifications: (reservation as any).enableNotifications || false,
                    notificationWhatsappEntry: (reservation as any).notificationWhatsappEntry || false,
                    notificationWhatsappExit: (reservation as any).notificationWhatsappExit || false
                });
            ...
            error: (error) => {
                this._errorHandlerService.showErrorAlert('Error', 'Failed to load reservation');
    ...
    /**
     * Sync startDate from arrivalDate (set time to midnight)
     */
    syncStartDate(): void {
        const date = this.reservationForm.get('arrivalDate').value;
        
        if (date) {
            const dateObj = new Date(date);
            dateObj.setHours(0, 0, 0, 0);
            this.reservationForm.patchValue({ startDate: dateObj }, { emitEvent: false });
        }
    }

    /**
     * Sync endDate from departureDate (set time to midnight)
     */
    syncEndDate(): void {
        const date = this.reservationForm.get('departureDate').value;
        
        if (date) {
            const dateObj = new Date(date);
            dateObj.setHours(0, 0, 0, 0);
            this.reservationForm.patchValue({ endDate: dateObj }, { emitEvent: false });
        }
    }
    ...
    saveReservation(): void {
        ...
        this.isLoading = true;
        const reservationValue = this.reservationForm.value;
        const additionalValue = this.additionalInfoForm.value;
        const notificationsValue = this.notificationsForm.value;

        // Ensure dates are set to midnight
        const startDate = new Date(reservationValue.startDate);
        startDate.setHours(0, 0, 0, 0);
        
        const endDate = new Date(reservationValue.endDate);
        endDate.setHours(0, 0, 0, 0);

        // Check for overlapping reservations before saving
        const excludeReservationId = this.isEditMode ? this.reservationId : undefined;
        this._reservationService.checkOverlappingReservations(
            reservationValue.propertyId,
            startDate.toISOString(),
            endDate.toISOString(),
            excludeReservationId
        ).subscribe({
            next: (overlappingReservations) => {
                if (overlappingReservations && overlappingReservations.length > 0) {
                    // Build error message with list of overlapping reservations
                    let errorMessage = `There are ${overlappingReservations.length} existing reservation(s) that overlap with the selected dates:\n\n`;
                    ...
                    overlappingReservations.forEach((reservation, index) => {
                        ...
                        const startDateStr = resStartDate.toLocaleDateString();
                        const endDateStr = resEndDate.toLocaleDateString();
                        const contactName = reservation.contactName || 'Unknown';
                        const status = reservation.status || 'Unknown';
                        
                        errorMessage += `${index + 1}. ${contactName} - ${startDateStr} to ${endDateStr} (Status: ${status})\n`;
                    });
                    
                    errorMessage += '\n\nPlease select different dates or contact the existing reservation holders.';
                    
                    // Show persistent error alert
                    this._errorHandlerService.showPersistentErrorAlert('Overlapping Reservations', errorMessage);
                    this.isLoading = false;
                    this._changeDetectorRef.markForCheck();
                    return;
                }
                ...
                this.proceedWithSave(reservationValue, additionalValue, notificationsValue, startDate, endDate);
            },
            error: (error) => {
    ...
    /**
     * Proceed with saving the reservation after overlap check
     */
    private proceedWithSave(reservationValue: any, additionalValue: any, notificationsValue: any, startDate: Date, endDate: Date): void {
        const reservationData = {
            contactId: reservationValue.contactId,
            propertyId: reservationValue.propertyId,
            startDate: startDate.toISOString(),
            endDate: endDate.toISOString(),
            reason: reservationValue.reason,
            description: reservationValue.description,
            status: additionalValue.status || 0,
            approvalNotes: additionalValue.approvalNotes,
            privateNote: additionalValue.privateNote,
            totalAmount: reservationValue.totalAmount,
            specialTerms: additionalValue.specialTerms,
            enableNotifications: notificationsValue.enableNotifications,
            notificationWhatsappEntry: notificationsValue.notificationWhatsappEntry,
            notificationWhatsappExit: notificationsValue.notificationWhatsappExit
        };
        ...
        if (this.isEditMode) {
            ...
            this._reservationService.updateReservation(this.reservationId, updateDto).subscribe({
                next: (response) => {
                    this._errorHandlerService.showSuccessAlert('Success', 'Reservation updated successfully');
                    this.isLoading = false;
                ...
                error: (error) => {
                    this._errorHandlerService.showErrorAlert('Error', 'Failed to update reservation');
            ...
            const createDto: CreateReservationDto = {
                ...reservationData as CreateReservationDto,
                attachments: this.attachmentsToAdd.length > 0 ? this.attachmentsToAdd : undefined
            };

            this._reservationService.createReservation(createDto).subscribe({
                next: (response) => {
                    this._errorHandlerService.showSuccessAlert('Success', 'Reservation created successfully');
                    this.isLoading = false;
                ...
                error: (error) => {
                    this._errorHandlerService.showErrorAlert('Error', 'Failed to create reservation');
     ...
     */
    viewAttachment(attachment: ReservationAttachment): void {
        if (!attachment.url) {
            return;
        }
     ...
     * Download attachment
     */
    downloadAttachment(attachment: ReservationAttachment, event: Event): void {
    ...
    /**
     * Get all images from reservation attachments
     */
    ...
    toggleEditMode(enable: boolean): void {
        ...
        if (enable) {
            ...
            // When disabling edit mode (canceling), reload the reservation to reset any changes
            if (this.isViewMode && this.reservationId) {
                this.loadReservation(this.reservationId);
            }
        }
     ...
     * These fields cannot be edited once the reservation is created
     */
    isPropertyTenantReadOnly(): boolean {
        return this.isEditMode; // Always read-only if editing existing reservation
    }
